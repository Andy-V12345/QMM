...
//    }
    ...
    var canvasView: some View {
        Canvas { context, size in
            for line in lines {
                ...
                context.stroke(path, with: .color(line.color), style: StrokeStyle(lineWidth: 7, lineCap: .round, lineJoin: .round))
            }
        }
        .gesture(
            DragGesture(minimumDistance: 0, coordinateSpace: .local)
                .onChanged({ value in
                    ...
                    if value.translation == .zero {
                        lines.append(Line(points: [position], color: color))
                    }
                    else {
    ...
    @State var captured: UIImage?
    @State var newCapturedI: Image?
    @State var newCaptured: UIImage?
    
    @State var imageName: String = "seven2.jpeg"
    
    @State var newDigit: String?
    ...
    var body: some View {
        ...
        VStack {
            ...
            processText(image: UIImage(named: imageName)!)
...
//        }
    ...
    fileprivate func processText(image: UIImage) -> some View {
...
//                isOn.toggle()
                ...
                let formattedImage = image.cgImage!
